...
class GMaps{
	 ...
	 * @return int Distance in meters (if the flag is true) or its "equivalent" imperial value (calculated by multiplying meters by 0.621371)
	 *
	...
	public function calculateDistance($locationA, $locationB, $metric = false){
		...
		if($data->rows[0]->elements[0]->status == 'NOT_FOUND'){ throw new \Exception("GoogleMaps returned: ADDRESS NOT FOUND", 1); }
		if($data->rows[0]->elements[0]->status == 'ZERO_RESULTS'){ return 9999999; }
