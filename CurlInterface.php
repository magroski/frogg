...
class CurlInterface{
	 ...
	 */
	public function postJson(string $url, $data=array(), $dataQuery=array()){
		...
		} elseif($this->authType==self::SAFE) {
	 ...
	 * @return string request result
	 */
	public function postUrl(string $url, $data=array(), $dataQuery=array()){
		$params = implode('/',$data);
		$query = http_build_query($dataQuery);
	 ...
	 * @return string request result
	 */
	public function getUrl(string $url, $data=array(), $dataQuery=array()){
		$params = implode('/',$data);
		$query = http_build_query($dataQuery);
		$ch = curl_init($url.$params.'?'.$query);
		curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true);
	 ...
	 * @return string request result
	 */
	public function getQuery(string $url, $data=array(), $dataQuery=array()){
	 ...
	 * @return string request result
	 */
	public function deleteReq(string $url, $data=array(), $dataQuery=array()){
		$query = http_build_query($dataQuery);
		$params = implode('/',$data);
		$ch = curl_init($url.$params.'?'.$query);
	 ...
	 * @return string request result
	 */
	public function putReq(string $url, $data=array(), $dataQuery=array()){
		$query = http_build_query($dataQuery);
		$ch = curl_init($url.'?'.$query);
